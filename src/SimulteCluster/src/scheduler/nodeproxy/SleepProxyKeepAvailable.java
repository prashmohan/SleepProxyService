...
public class SleepProxyKeepAvailable implements NodeProxy {
	...
	// kept available
	public SleepProxyKeepAvailable(List<Node> nodes, double perKeepAvail) {
		this.nodes = nodes;
		if (!nodes.isEmpty()) {
			masterNode = nodes.get(0);
		}
		// number of nodes that must be kept available
		numKeepAvail = (int)(perKeepAvail * nodes.size());
	...
	public List<Command> getSleepCommands(int time) {
		...
		for (Node node : availableNodes) {
			// ensure that numKeep nodes are available
			if (availableNodes.size() - nodesPutSleep.size() < numKeepAvail) {
				break;
			}
	...
	public List<Node> getAvailableNodes(int numNodesRequired, int time) {
		...
		// scheduler always requires numKeep extra nodes
		numNodesRequired += numKeepAvail;
